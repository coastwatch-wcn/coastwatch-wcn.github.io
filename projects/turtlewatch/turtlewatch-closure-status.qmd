---
pagetitle: "Conservation Area Status"
format: 
  html:
    theme: lux
    page-layout: full
...
![Map of the Pacific Loggerhead Conservation Area (Loggerhead Closure). Map copyright: NOAA Fisheries](images/loggerhead_conservation.png){width="60%"}
...
![](data/images/indicator_latest.png){#indicator-plot fig-align="center" width="70%"}
:::
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // === True for Closure, False for open ===
  const CLOSURE_ACTIVE = true;
  ...
  fetch("data/resources/ltca_closure.csv")  
    ...
    .then(text => {
      ...
      // Manual banner switch
      banner.classList.remove("success", "alert-active", "alert-upcoming");
  ...
  fetch("data/json/heatwave.json")
    ...
    .then(data => {
      ...
      const regions  = document.getElementById("heat-regions");
      ...
      if (regions) {
        ...
        if (data.pacific_regions &&
            Object.keys(data.pacific_regions).length > 0) {

          regions.innerHTML = "";

          Object.entries(data.pacific_regions).forEach(([region, forecast]) => {
            ...
            regions.appendChild(p);

          });

        } else {

          regions.innerHTML = "<p>No Pacific forecasts available.</p>";
